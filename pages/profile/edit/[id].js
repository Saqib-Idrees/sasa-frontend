...
export default function Edit() {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      debugger;
      const response = await updateProfileById({id:userData.id,...values}).unwrap();
      if (isError === false) {
        Swal.fire({
          position: "top-end",
          icon: "success",
  ...
  useEffect(() => {
    if (userData) {
      setIsTailor(userData?.role === "Tailor" ? true : false);
    }
  }, [userData]);

  return (
    <div>
      {userIsLoading || userIsFetching || userData === undefined ? (
        <Layout>
      ...
      {userIsError && <h5>No posts found</h5>}
    </div>
  );
}
